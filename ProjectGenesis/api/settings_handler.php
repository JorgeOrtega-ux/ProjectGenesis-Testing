...
if (!isset($_SESSION['user_id'])) {
    $response['message'] = __('api.accessDenied');
        ...
        if ($action === 'upload-avatar') {
            try {
                if (!isset($_FILES['avatar']) || $_FILES['avatar']['error'] !== UPLOAD_ERR_OK) {
                    throw new Exception(__('settings.avatar.error.upload', ['code' => ($_FILES['avatar']['error'] ?? 'N/A')]));
                }
                ...
                if ($fileSize > 2 * 1024 * 1024) {
                    throw new Exception(__('settings.avatar.error.size'));
                }
                ...
                if (!array_key_exists($mimeType, $allowedTypes)) {
                    throw new Exception(__('settings.avatar.error.format'));
                }
                ...
                if (!move_uploaded_file($fileTmpName, $newFilePath)) {
                    throw new Exception(__('settings.avatar.error.save'));
                }
                ...
                $response['message'] = __('settings.avatar.success.uploaded');
            ...
            try {
                ...
                if (!$newDefaultUrl) {
                    throw new Exception(__('settings.avatar.error.generateDefault'));
                }
                ...
                $response['success'] = true;
                $response['message'] = __('settings.avatar.success.removed');
                $response['newAvatarUrl'] = $newDefaultUrl;
            } catch (Exception $e) {
                if ($e instanceof PDOException) {
                    logDatabaseError($e, 'settings_handler - remove-avatar');
                    $response['message'] = __('api.dbErrorUpdate');
                } else {
                    $response['message'] = $e->getMessage();
                }
            }
            ...
            try {
                ...
                if ($lastLog) {
                    ...
                    if ($daysPassed < USERNAME_CHANGE_COOLDOWN_DAYS) {
                        ...
                        // Nota: La pluralización real debería manejarse en el JSON
                        $plural = ($daysRemaining == 1) ? __('settings.username.error.cooldownDay') : __('settings.username.error.cooldownDays');
                        throw new Exception(__('settings.username.error.cooldown', ['days' => $daysRemaining, 'plural' => $plural]));
                    }
                }
                ...
                if (empty($newUsername)) {
                    throw new Exception(__('settings.username.error.empty'));
                }
                if (strlen($newUsername) < MIN_USERNAME_LENGTH) {
                    throw new Exception(__('api.register.error.usernameTooShort', ['min' => MIN_USERNAME_LENGTH]));
                }
                if (strlen($newUsername) > MAX_USERNAME_LENGTH) {
                    throw new Exception(__('api.register.error.usernameTooLong', ['max' => MAX_USERNAME_LENGTH]));
                }

                if ($newUsername === $oldUsername) {
                    throw new Exception(__('settings.username.error.noChange'));
                }

                $stmt = $pdo->prepare("SELECT id FROM users WHERE username = ? AND id != ?");
                $stmt->execute([$newUsername, $userId]);
                if ($stmt->fetch()) {
                    throw new Exception(__('api.register.error.usernameInUse'));
                }
                ...
                $response['success'] = true;
                $response['message'] = __('settings.username.success');
            ...
            try {
                ...
                $response['success'] = true;
                $response['message'] = __('settings.email.codeGenerated');
            } catch (Exception $e) {
                if ($e instanceof PDOException) {
                    logDatabaseError($e, 'settings_handler - request-email-change-code');
                    if (strpos($e->getMessage(), 'Data truncated') !== false) {
                        $response['message'] = __('settings.email.error.enum', ['codeType' => $codeType]);
                    } else {
                        $response['message'] = __('api.dbError');
                    }
                } else {
            ...
            try {
                ...
                if (checkLockStatus($pdo, $identifier, $ip)) {
                    throw new Exception(__('api.login.error.tooManyAttempts', ['minutes' => LOCKOUT_TIME_MINUTES]));
                }


                if (empty($submittedCode)) {
                    throw new Exception(__('settings.email.error.emptyCode'));
                }
                ...
                if (!$codeData || strtolower($codeData['code']) !== strtolower($submittedCode)) {
                    ...
                    throw new Exception(__('api.reset.error.invalidCode'));
                }
                ...
                $response['success'] = true;
                $response['message'] = __('settings.email.success.verified');
            } catch (Exception $e) {
                if ($e instanceof PDOException) {
            ...
            try {
                ...
                if ($lastLogEmail) {
                    ...
                    if ($daysPassed < EMAIL_CHANGE_COOLDOWN_DAYS) {
                        ...
                        $plural = ($daysRemaining == 1) ? __('settings.email.error.cooldownDay') : __('settings.email.error.cooldownDays');
                        throw new Exception(__('settings.email.error.cooldown', ['days' => $daysRemaining, 'plural' => $plural]));
                    }
                }
                ...
                if (empty($newEmail) || !filter_var($newEmail, FILTER_VALIDATE_EMAIL)) {
                    throw new Exception(__('api.register.error.invalidEmailFormat'));
                }
                if (strlen($newEmail) > MAX_EMAIL_LENGTH) {
                    throw new Exception(__('api.register.error.emailTooLong', ['max' => MAX_EMAIL_LENGTH]));
                }

                if ($newEmail === $oldEmail) {
                    throw new Exception(__('settings.email.error.noChange'));
                }
                ...
                if (!in_array(strtolower($emailDomain), $allowedDomains)) {
                    throw new Exception(__('api.register.error.domainNotAllowed'));
                }
                ...
                if ($stmt->fetch()) {
                    throw new Exception(__('api.register.error.emailInUse'));
                }
                ...
                $response['success'] = true;
                $response['message'] = __('settings.email.success.updated');
            ...
            try {
                ...
                if (empty($currentPassword)) {
                    throw new Exception(__('settings.password.error.emptyCurrent'));
                }
                ...
                if ($hashedPassword && password_verify($currentPassword, $hashedPassword)) {
                    clearFailedAttempts($pdo, $identifier);
                    $response['success'] = true;
                    $response['message'] = __('settings.password.success.verified');
                } else {
                    logFailedAttempt($pdo, $identifier, $ip, 'password_verify_fail');
                    throw new Exception(__('settings.password.error.wrongCurrent'));
                }
            } catch (Exception $e) {
                if ($e instanceof PDOException) {
            ...
            try {
                ...
                if ($lastLogPass) {
                    ...
                    if ($hoursPassed < PASSWORD_CHANGE_COOLDOWN_HOURS) {
                        ...
                        $plural = ($hoursRemaining == 1) ? __('settings.password.error.cooldownHour') : __('settings.password.error.cooldownHours');
                        throw new Exception(__('settings.password.error.cooldown', ['hours' => $hoursRemaining, 'plural' => $plural]));
                    }
                }
                ...
                if (empty($newPassword) || empty($confirmPassword)) {
                    throw new Exception(__('settings.password.error.emptyNew'));
                }
                if (strlen($newPassword) < MIN_PASSWORD_LENGTH) {
                    throw new Exception(__('api.register.error.passwordTooShort', ['min' => MIN_PASSWORD_LENGTH]));
                }
                if (strlen($newPassword) > MAX_PASSWORD_LENGTH) {
                    throw new Exception(__('api.register.error.passwordTooLong', ['max' => MAX_PASSWORD_LENGTH]));
                }
                if ($newPassword !== $confirmPassword) {
                    throw new Exception(__('settings.password.error.mismatch'));
                }
            ...
            try {
                ...
                $response['message'] = $newState === 1
                    ? __('settings.2fa.success.enabled')
                    : __('settings.2fa.success.disabled');
            } catch (Exception $e) {
                if ($e instanceof PDOException) {
            ...
            try {
                ...
                $allowedFields = [
                    'language' => ['en-us', 'fr-fr', 'es-latam', 'es-mx', 'es-419'], // Añadido es-419
                ...
                if (!array_key_exists($field, $allowedFields)) {
                    throw new Exception(__('settings.prefs.error.invalidField'));
                }

                if (!in_array($value, $allowedFields[$field])) {
                    if ($field === 'open_links_in_new_tab' || $field === 'increase_message_duration') {
                        throw new Exception(__('settings.prefs.error.invalidToggleValue'));
                    }
                    throw new Exception(__('settings.prefs.error.invalidValue'));
                }
                ...
                $response['success'] = true;
                $response['message'] = __('settings.prefs.success');
            } catch (Exception $e) {
                if ($e instanceof PDOException) {
            ...
            try {
                ...
                $response['success'] = true;
                $response['message'] = __('settings.devices.logoutAllSuccess');
            } catch (Exception $e) {
                if ($e instanceof PDOException) {
